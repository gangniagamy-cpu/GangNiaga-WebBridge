...
description: 'Run and orchestrate the local test agent (test-agent-aku-win.js) from native Windows (PowerShell or cmd.exe), communicating with the WebBridge Daemon running locally on localhost.'
---

# 🤖 GANGNIAGA TEST AGENT WINDOWS — NATIVE AUTOMATION PROTOCOL

This skill provides operational parameters, configuration settings, and command reference for executing the Node.js **Local WebBridge Test Agent** (`test-agent-aku-win.js`) natively on Windows.

---

## 🔌 1. Architecture & Native Communication

When running natively on Windows, both the Local Test Agent and the WebBridge Daemon execute on the same machine. Communication is direct and utilizes the loopback address.

```
┌──────────────────────────────────────┐
│        Windows Native System         │
│  ┌────────────────────────────────┐  │
│  │   Local Test Agent (Node.js)   │  │
│  └───────────────┬────────────────┘  │
...
To run the local test agent natively on Windows, you must open a terminal (PowerShell or Command Prompt) and run NPM scripts:
