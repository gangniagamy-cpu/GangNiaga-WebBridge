    ...
    tools: [
      {
        name: 'browser_status',
        description:
          'Check if the GangNiaga WebBridge daemon and Chrome extension are running and connected.',
        inputSchema: { type: 'object', properties: {} },
      },
      {
        name: 'browser_get_site_rules',
        description:
          'Fetch the YAML site knowledge base (selectors and recipes) for a specific domain (e.g. shopee.com.my). ALWAYS run this before interacting with a new website to avoid hallucinating selectors.',
        inputSchema: {
          type: 'object',
          properties: {
      ...
      },
      {
        ...
        description:
          'Navigate the browser to a specific URL. Can open in a new tab if newTab is true. Returns tabId.',
        inputSchema: {
          type: 'object',
          properties: {
      ...
      },
      {
        ...
        description:
          'Get the simplified Accessibility Tree (AXTree) of the current page. Returns elements with ref strings (like @e1) to be used with click/fill.',
        inputSchema: { type: 'object', properties: {} },
      },
      {
        name: 'browser_click',
        description:
          "Click an element on the page using a CSS selector or ref string (e.g., '@e1'). Supports AI self-healing.",
        inputSchema: {
          type: 'object',
          properties: {
            ...
            _approved: {
              type: 'boolean',
              description:
                'Set to true to bypass sensitive action warning if prompt asks to proceed with confirmation',
            },
          },
          required: ['selector'],
        },
      },
      {
        name: 'browser_fill',
        description:
          'Fill an input field with text using a CSS selector or ref string. Supports AI self-healing.',
        inputSchema: {
          type: 'object',
          properties: {
            selector: { type: 'string', description: 'CSS selector or @e ref' },
      ...
      },
      {
        ...
        description:
          'Move the mouse to an element using Bezier curve path and click it at the coordinates (simulates human movement to bypass anti-bots).',
        inputSchema: {
          type: 'object',
          properties: {
            selector: { type: 'string', description: 'CSS selector or @e ref' },
            _approved: {
              type: 'boolean',
              description: 'Set to true to bypass sensitive action warning',
            },
          },
          required: ['selector'],
        },
      },
      {
        name: 'browser_scroll',
        description:
          'Scroll the page. Can scroll by delta coordinates or scroll to a specific element.',
        inputSchema: {
          type: 'object',
          properties: {
      ...
      },
      {
        ...
        description:
          'Wait for a condition to be met (element visible, network idle, or time delay).',
        inputSchema: {
          type: 'object',
          properties: {
            ...
            state: {
              type: 'string',
              description: 'Optional state to wait for: visible, hidden, detached',
            },
          },
        },
      },
      ...
      },
      {
        name: 'os_click',
        description:
          'Perform an OS-level mouse click at exact screen coordinates (x, y) via PowerShell.',
        inputSchema: {
          type: 'object',
          properties: {
      ...
      },
      {
        name: 'os_screenshot',
        description:
          'Capture a full screenshot of the primary monitor using OS APIs and save it locally.',
        inputSchema: {
          type: 'object',
          properties: {
            path: {
              type: 'string',
              description: 'Absolute path to save screenshot (e.g. C:\\temp\\screen.png)',
            },
          },
          required: ['path'],
        },
      },
      {
        name: 'os_hotkey',
        description:
          "Press a keyboard shortcut/hotkey combination (e.g. ['ctrl', 't'] or ['alt', 'tab']).",
        inputSchema: {
          type: 'object',
          properties: {
            keys: {
              type: 'array',
              items: { type: 'string' },
              description: "Array of key strings, e.g., ['ctrl', 'shift', 't']",
            },
          },
          required: ['keys'],
        },
      },
      {
        ...
        description:
          'Simulate typing a text string character-by-character with random human-like delays and occasional self-correcting typos.',
        inputSchema: {
          type: 'object',
          properties: {
      ...
      },
      {
        ...
        description:
          "Send modifier keys and special key sequences (e.g. 'Control+a', 'Enter', 'Escape') to the browser page.",
        inputSchema: {
          type: 'object',
          properties: {
            keys: {
              type: 'string',
              description: "Key sequence or combination, e.g., 'Control+a' or 'Enter'",
            },
          },
          required: ['keys'],
        },
