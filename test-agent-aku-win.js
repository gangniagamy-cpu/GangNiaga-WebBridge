async function runTest() {
  ...
  console.log('[GangNiaga-Test] Memulakan simulasi autonomus...');

  // 1. Fetch YAML Knowledge Base
  console.log('[GangNiaga-Test] Sedang mengambil Knowledge Base untuk Shopee...');
  ...
  console.log(
    '[GangNiaga-Test] YAML Knowledge dijumpai:\n',
    siteData.yaml.split('\n').slice(0, 5).join('\n') + '\n  ...\n',
  );
  ...
  console.log(`[GangNiaga-Test] Faham! Selector search bar ialah: ${searchInputSelector}`);

  // 2. Open Shopee Tab (use 'navigate' with newTab=true, correct action name)
  console.log('[GangNiaga-Test] Mengarahkan WebBridge untuk buka tab Shopee...');
  ...
  if (!cmdData.success) {
    ...
    console.log('[GangNiaga-Test] Tab berjaya dibuka!');
  }
  ...
  console.log('[GangNiaga-Test] Screenshot result:', ssData.success ? 'OK' : ssData.error);

  // 4. Use evaluate (not run_js) with React-compatible value setter
  console.log("[GangNiaga-Test] Menjalankan recipe: Type 'Laptop' & Click Search");
  const jsPayload = `
    (function() {
  ...
  console.log('[GangNiaga-Test] Hasil pelaksanaan di browser:', evalData);
  console.log('[GangNiaga-Test] Simulasi selesai!');
}

runTest().catch((e) => console.error('Fatal:', e.message));
