...
**GangNiaga WebBridge** ialah ejen perantara (*bridge daemon*) ringan yang membolehkan ejen kecerdasan buatan (seperti Hermes, Claude Desktop, Cursor, LangChain) mengawal pelayar Google Chrome sedia ada anda.
...
4. Muat semula profil terminal shell anda:
   ...
   ```
5. Ini akan mencipta alias `hermes` dan `hermes-i` dalam WSL anda secara kekal.
...
* **"Browser assistant is not ready"**: Tiada sambungan. Ini berlaku jika:
  ...
  2. Service Worker ditidurkan (suspend) oleh Chrome semasa tiada kerja (rujuk bab *Troubleshooting*).
...
## 🦅 5. Cara Menjalankan Ejen Automasi (Hermes Agent)

Terdapat dua cara utama untuk memulakan ejen automasi Hermes:
...
Sesuai untuk menguji arahan satu persatu secara langsung (*step-by-step*).
* **Dari WSL Kali/Ubuntu**:
  ```bash
  hermes-i
  ```
...
| `snapshot` | `{}` | Mengambil data pokok elemen (*Accessibility Tree - AXTree*). |
...
---
*Manual ini diselenggara oleh GangNiaga AI Team. Sila rujuk [AGENTS.md](file:///D:/GangNiaga-WebBridge/AGENTS.md) untuk perincian penulisan skrip ejen automasi.*
