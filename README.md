...
**GangNiaga WebBridge** ialah jambatan automasi pelayar web berperisa perusahaan (*enterprise-grade*) yang menghubungkan ejen AI tempatan (seperti Hermes, PUSPA-V4, Claude Desktop, Cursor) terus ke profil Google Chrome Windows anda yang aktif dan disahkan.
...
* 🐧 **Pautan Telus WSL-ke-Windows**: Ejen AI di dalam WSL Kali Linux/Ubuntu boleh mengesan IP Windows Host secara dinamik dan memandu Chrome dengan sifar kelewatan (*zero latency*).
...
graph TD
    subgraph WSL2 [WSL 2 Terminal - Linux]
        A[Hermes Agent CLI] -- "HTTP / WebSocket" --> B[Network Bridge Gateway]
    end
...
*Pastikan tetingkap daemon terus berjalan di latar belakang.*
...
# Jalankan Ejen Automasi
hermes
```

---

## 🎮 Arahan Ejen Automasi (Hermes CLI Console)

WebBridge dilengkapi dengan **Hermes Agent CLI** untuk kawalan interaktif terus dari terminal anda.

### 🕹️ Mod Interaktif (`hermes-i`)
...
| | `mouse_click` | `{"selector": "div.btn-buy"}` *(Bypass Anti-Bot)* |
...
| | `hotkey` | `{"keys": ["alt", "tab"]}` |
| **Pengambilan Data**| `snapshot` | `{}` *(Mengembalikan AXTree)* |
| | `screenshot` | `{"selector": "div#chart"}` *(Screenshot DOM)* |
...
├── skills/         # Definisi Arahan Ejen AI (Hermes WSL & Windows)
...
*Dibina dengan penuh dedikasi oleh **GangNiaga AI Developer Team** untuk generasi automasi ejen AI masa hadapan.*
