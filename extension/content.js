    ...
    switch (request.action) {
      case 'getDOMInfo':
        sendResponse({
          ...
          forms: Array.from(queryAllElementsDeep('form')).map((f) => ({
            id: f.id,
            action: f.action,
          })),
          inputs: Array.from(queryAllElementsDeep('input, textarea'))
            .slice(0, 20)
            .map((i) => ({ type: i.type, id: i.id, placeholder: i.placeholder })),
          images: Array.from(queryAllElementsDeep('img'))
            .slice(0, 10)
            .map((i) => ({ src: i.src, alt: i.alt })),
          links: Array.from(queryAllElementsDeep('a[href]'))
            .slice(0, 10)
      ...
      case 'getPageStructure':
        const structure = {
          url: window.location.href,
          title: document.title,
          headings: Array.from(queryAllElementsDeep('h1,h2,h3')).map((h) => ({
            level: h.tagName,
            text: h.textContent.trim(),
          })),
          sections: Array.from(queryAllElementsDeep('section, article, [role="main"]')).map(
            (s) => ({ tag: s.tagName, id: s.id, className: s.className }),
          ),
          navLinks: Array.from(queryAllElementsDeep('nav a, [role="navigation"] a'))
            .slice(0, 10)
