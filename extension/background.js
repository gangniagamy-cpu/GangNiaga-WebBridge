var background = (function () {
  ...
  async function reportSelfHealing(domain, originalSelector, healedSelector, healedIndex, tag) {
    ...
    try {
      ...
      console.log(
        `[self-healing] reported healing to daemon: ${originalSelector} -> ${healedSelector}`,
      );
    } catch (e) {
  ...
  async function trySelfHealingClick(selector) {
    ...
    try {
      ...
      let elementsList = elements
        .map(
          (el) =>
            `Index ${el.index}: <${el.tag} id="${el.id}" class="${el.className}" placeholder="${el.placeholder}">${el.text}</${el.tag}>`,
        )
        .join('\n');
...
Which index is the closest semantic match to the target input field? Reply with ONLY the index number (e.g. 3) and nothing else. If no element matches, reply "NONE".`;
  ...
  async function moveMouseHumanLike(targetX, targetY) {
    ...
    for (const pt of points) {
      await a(`Input.dispatchMouseEvent`, {
        type: `mouseMoved`,
        x: Math.round(pt.x),
        y: Math.round(pt.y),
        button: `none`,
  ...
  async function setCryptoSecret(secretText) {
    ...
    cryptoSecretKey = await crypto.subtle.importKey('raw', hash, { name: 'AES-GCM' }, false, [
      'encrypt',
      'decrypt',
    ]);
  }
  ...
  (chrome.tabs.onRemoved.addListener((e) => {
    (t.delete(e), n === e && (n = null), r === e && (r = null));
  }),
    chrome.debugger.onDetach.addListener((e) => {
      e.tabId && (t.delete(e.tabId), n === e.tabId && (n = null));
    }));
  ...
  async function a(e, t) {
    if (n === null) throw Error(`No tab attached. Call attach(tabId) first.`);
    return await chrome.debugger.sendCommand({ tabId: n }, e, t);
  }
  function o() {
    return n;
  }
  async function s() {
    ...
    if (r !== null)
      try {
        let e = await chrome.tabs.get(r);
        if (e) return e;
      } catch {
        r = null;
      }
    let [e] = await chrome.tabs.query({ active: !0, currentWindow: !0 });
    if (!e?.id) throw Error(`No active tab found`);
    return ((r = e.id), e);
  }
  function c(e) {
    r = e;
  ...
  // Tab grouping
  var u = { twitter: `blue`, xhs: `red`, zhihu: `blue`, worldquant: `purple` },
    d = [`green`, `yellow`, `cyan`, `orange`, `pink`, `grey`],
    f = new Map(),
    p = new Map(),
    m = 0,
    h = !1;
  ...
  async function g(e, t, n) {
    try {
      let r = f.get(t);
      if (r != null) {
        await chrome.tabs.group({ tabIds: e, groupId: r });
        return;
      }
      let i = `agent:${t}`,
        a = await chrome.tabGroups.query({ title: i });
      if (a.length > 0) {
        let n = a[0].id;
        (await chrome.tabs.group({ tabIds: e, groupId: n }), f.set(t, n));
        return;
      }
      n && p.set(t, n);
      let o = n ?? p.get(t) ?? i,
        s = await chrome.tabs.group({ tabIds: e }),
        c = u[t] ?? d[m++ % d.length];
      (await chrome.tabGroups.update(s, { title: o, color: c, collapsed: !1 }), f.set(t, s));
    } catch {}
  }
  ...
  var _ = class {
      ...
      async execute(e) {
        ...
        if (r)
          return (
            (n = await chrome.tabs.create({ url: t, active: !0 })),
            c(n.id),
            o && (await g(n.id, o, l)),
            await i(n.id),
            await this.waitForLoad(n.id),
            { success: !0, url: t, tabId: n.id }
          );
        if (((n = await s()), n.url?.startsWith(`chrome://`) || n.url?.startsWith(`edge://`)))
          return (
            (n = await chrome.tabs.create({ url: t, active: !0 })),
            c(n.id),
            await this.waitForLoad(n.id),
        ...
        (await i(n.id), c(n.id));
        let u = n.url === t || n.url === t + `/`,
          d;
        return (
          u
            ? await a(`Page.reload`, { ignoreCache: !0 })
            : (d = (await a(`Page.navigate`, { url: t })).frameId),
          await this.waitForLoad(n.id),
          { success: !0, url: t, tabId: n.id, frameId: d }
        );
      }
      waitForLoad(e) {
        return new Promise((t, n) => {
          let r = setTimeout(() => {
              (chrome.tabs.onUpdated.removeListener(a),
                n(Error(`navigate: page load timeout (30s)`)));
            }, 3e4),
            ...
            a = (n, o, s) => {
              n === e &&
                o.status === `complete` &&
                i(s) &&
                (clearTimeout(r), chrome.tabs.onUpdated.removeListener(a), t());
            };
          chrome.tabs.get(e, (e) => {
            i(e) ? (clearTimeout(r), t()) : chrome.tabs.onUpdated.addListener(a);
          });
        });
      }
    },
    v = class {
      ...
      async execute(e) {
        ...
        if (
          (r &&
            (o = (
              await chrome.windows.getLastFocused({ populate: !0, windowTypes: [`normal`] })
            ).tabs?.find((e) => e.active && e.url && te(e.url, a))),
          (o ||= (await chrome.tabs.query({ url: a }))[0]),
          !o)
        )
          throw Error(
            `find_tab: no open tab found matching ${t} — open the page first, or use navigate to open it`,
          );
        let s = o.id;
  ...
  function te(e, t) {
    try {
      return new URL(e).hostname === t.replace(/^\*:\/\//, ``).replace(/\/\*$/, ``);
    } catch {
      return !1;
    }
  }
  ...
  var ne = class {
      ...
      async execute(e) {
        ...
        if (n.exceptionDetails) {
          let e = n.exceptionDetails.exception?.description || n.exceptionDetails.text;
          throw Error(`evaluate: ${e}`);
        }
  ...
  var C = class e {
      ...
      async execute(e) {
        ...
        switch (t) {
          ...
          case `stop`:
            return this.stop();
          case `list`:
            return this.list(e.filter);
          case `detail`:
            return this.detail(e.requestId);
          default:
            throw Error(`network: unknown cmd "${t}"`);
        }
      }
      async start() {
        let t = await s();
        ...
        return (
          ...
          (e._listenerAdded ||=
            (chrome.debugger.onEvent.addListener((e, t, n) => {
              let r = e.tabId;
              if (!r || !b.has(r)) return;
              let i = S(r);
              if (
                (t === `Network.requestWillBeSent` &&
                  i.set(n.requestId, {
                ...
                t === `Network.responseReceived`)
              ) {
                let e = i.get(n.requestId);
                e && ((e.status = n.response.status), (e.mimeType = n.response.mimeType));
              }
              if (t === `Network.loadingFinished`) {
                let e = i.get(n.requestId);
                e && (e.completed = !0);
              }
            }),
            !0)),
          { success: !0, message: `network capture started` }
        );
      }
      async stop() {
        let e = o();
      ...
      list(e) {
        let t = o(),
          n = [...(t === null ? new Map() : S(t)).values()];
        return (
          e && (n = n.filter((t) => t.url.includes(e))),
          {
            count: n.length,
            requests: n.map((e) => ({
      ...
      async detail(e) {
        ...
        let t = o(),
          n = (t === null ? new Map() : S(t)).get(e);
        if (!n) throw Error(`network: request "${e}" not found`);
        let r = await a(`Network.getResponseBody`, { requestId: e }),
          i = r.body;
        if (!r.base64Encoded)
          try {
            i = JSON.parse(r.body);
  ...
  function ae(e, t, n) {
    let r = `e${T++}`;
    return (w.set(r, { backendDOMNodeId: e, role: t, name: n }), r);
  }
  function E(e) {
  ...
  var oe = class {
      ...
      async execute(e) {
        let t = await s();
        (await i(t.id), ie());
        let n = await a(`Accessibility.getFullAXTree`),
          r = this.buildTree(n.nodes);
        return { url: t.url, title: t.title, tree: r };
      }
      buildTree(e) {
        let t = new Map();
        for (let n of e) t.set(n.nodeId, n);
        return e.length === 0 ? [] : this.formatChildren(e[0], t);
      }
      formatChildren(e, t) {
        let n = [],
          r = (e) => {
            ...
            let i = { role: n };
            if (
              (e.name?.value && (i.name = e.name.value),
              e.value?.value && (i.value = e.value.value),
              e.description?.value && (i.description = e.description.value),
              re.has(n) &&
                e.backendDOMNodeId != null &&
                (i.ref = `@${ae(e.backendDOMNodeId, n, e.name?.value ?? ``)}`),
              e.childIds?.length)
            ) {
              let n = [];
              for (let i of e.childIds) {
    ...
    se = class {
      ...
      async clickByRef(e) {
        ...
        let r = await a(`Runtime.callFunctionOn`, {
          objectId: n.objectId,
          functionDeclaration: `function() {
    ...
    ce = class {
      ...
      async fillByRef(e, t) {
        ...
        let i = await a(`Runtime.callFunctionOn`, {
          objectId: r.objectId,
          functionDeclaration: `function() { ${O(`this`, t)} }`,
          returnByValue: !0,
        });
  ...
  var le = class {
      ...
      async execute(e) {
        ...
        let r;
        try {
          r = await a(`DOM.getBoxModel`, { objectId: n });
        } catch (e) {
          throw Error(
            `mouse_click: element has no layout box (display:none / detached / zero-size). Use 'click' for DOM-level fallback. (CDP: ${e.message})`,
          );
        }
        if (!r.model || !r.model.content || r.model.content.length < 8)
          throw Error(
            `mouse_click: element has no layout box (display:none / detached / zero-size). Use 'click' for DOM-level fallback.`,
          );
        let o = r.model.content,
          c = (o[0] + o[2] + o[4] + o[6]) / 4,
          l = (o[1] + o[3] + o[5] + o[7]) / 4;
        ...
        let u = await a(`Runtime.callFunctionOn`, {
          objectId: n,
        ...
        return {
          ...
          x: Math.round(c),
          y: Math.round(l),
          tag: u.result.value?.tag ?? ``,
          text: u.result.value?.text ?? ``,
        };
      }
      ...
      async objectIdFromSelector(e) {
        let params = {
          expression: `document.querySelector(${JSON.stringify(e)})`,
          returnByValue: !1,
        };
        let ctxId = getContextId();
        if (ctxId !== undefined) params.contextId = ctxId;
        ...
        if (t.result.subtype === `null` || !t.result.objectId)
          throw Error(`mouse_click: element not found: ${e}`);
        ...
        for (let char of t) {
          if (/[a-zA-Z]/.test(char) && Math.random() < 0.05) {
            ...
            if (possibleTypos) {
              ...
              await a(`Input.dispatchKeyEvent`, {
                type: 'rawKeyDown',
  ...
  async function N() {
    return (M === null && (M = (await chrome.runtime.getPlatformInfo()).os), M);
  }
  function P(e) {
    return e === `mac` ? A.cmd : A.ctrl;
  }
  ...
  function I(e) {
    ...
    if (n) {
      ...
      if (e >= 1 && e <= 12) return { key: `F${e}`, code: `F${e}`, vkc: 111 + e };
    }
    if (e.length === 1) {
      if (/^[a-zA-Z]$/.test(e)) {
        let t = e.toLowerCase(),
          n = e.toUpperCase();
        return { key: t, code: `Key${n}`, vkc: n.charCodeAt(0), text: t };
      }
      if (/^[0-9]$/.test(e)) return { key: e, code: `Digit${e}`, vkc: e.charCodeAt(0), text: e };
    }
    throw Error(
      `send_keys: unknown key "${e}". Supported: ${Object.keys(F).join(`, `)}, F1-F12, single letters/digits.`,
    );
  }
  ...
  function L(e, t) {
    ...
    for (let e = 0; e < n.length - 1; e++) {
      let a = n[e].toLowerCase(),
        o = a === `mod` ? t : A[a];
      if (o === void 0)
        throw Error(
          `send_keys: "${n[e]}" is not a modifier. Use Alt/Ctrl/Cmd/Meta/Shift, or Mod (auto-resolves to Cmd on Mac, Ctrl on Win/Linux).`,
        );
      ((r |= o.bit), i.push(o));
    }
  ...
  var z = class {
      ...
      async execute(e) {
        ...
        let n = e.repeat,
          r = n === void 0 ? 1 : Number(n);
        if (!Number.isInteger(r) || r < 1 || r > 100)
          throw Error(`send_keys: repeat must be an integer in [1, 100]`);
        let o = await N(),
          c = P(o),
          l = t
            .trim()
        ...
        let u = 0;
        for (let e = 0; e < r; e++)
          for (let { modifierBits: e, modifierKeys: t, spec: n } of l) {
            let r = R(n, (e & j) !== 0),
              i = 0;
            for (let e of t)
              ((i |= e.bit),
    ...
    B = class {
      ...
      async execute(e) {
        await i((await s()).id);
        let t = e.format || `png`,
          n = t === `jpeg` ? e.quality || 80 : void 0,
          r = typeof e.selector == `string` ? e.selector : ``,
          o = { format: t };
        if ((n !== void 0 && (o.quality = n), r)) {
          ...
          let t;
          try {
            t = await a(`DOM.getBoxModel`, { objectId: e });
          } catch (e) {
            throw Error(
              `screenshot: element has no layout box (display:none / detached / zero-size). (CDP: ${e.message})`,
            );
          }
          ...
          let i = [n[0], n[2], n[4], n[6]],
            ...
            d = Math.max(...s) - l;
          if (u <= 0 || d <= 0)
            throw Error(`screenshot: element has zero-size box (width=${u}, height=${d}).`);
      ...
      async objectIdFromSelector(e) {
        ...
        if (t.result.subtype === `null` || !t.result.objectId)
          throw Error(`screenshot: element not found: ${e}`);
        return t.result.objectId;
      }
    },
    V = {
    ...
    H = class {
      ...
      async execute(e) {
        await i((await s()).id);
        let [t, n] = V[(e.paper_format || `letter`).toLowerCase()] ?? V.letter,
          r = typeof e.scale == `number` ? e.scale : 1;
        if (r < 0.1 || r > 2) throw Error(`save_as_pdf: scale must be in [0.1, 2.0], got ${r}`);
        let o = await a(`Page.printToPDF`, {
          printBackground: e.print_background !== !1,
        ...
        try {
          c =
            (await a(`Runtime.evaluate`, { expression: `document.title`, returnByValue: !0 }))
              .result?.value ?? ``;
        } catch {}
        return {
          data: o.data,
    ...
    U = class {
      ...
      async execute(e) {
        ...
        if (!n || !Array.isArray(n) || n.length === 0)
          throw Error(`upload: files is required (array of local file paths)`);
        await i((await s()).id);
        let { nodeId: r } = await a(`DOM.querySelector`, {
          nodeId: (await a(`DOM.getDocument`)).root.nodeId,
          selector: t,
        });
        if (!r) throw Error(`upload: element not found: ${t}`);
        return (
          await a(`DOM.setFileInputFiles`, { files: n, nodeId: r }),
          { success: !0, selector: t, fileCount: n.length, files: n }
        );
      }
    },
    de = class {
      ...
      async execute(e) {
        ...
        try {
          return (await chrome.tabs.remove(t), { success: !0, closed: !0 });
        } catch {
          return { success: !0, closed: !1, reason: `tab already closed` };
        }
      }
    },
    fe = class {
        ...
        for (let e of t)
          try {
            ...
            if (t.groupId != null && t.groupId !== chrome.tabGroups.TAB_GROUP_ID_NONE)
              try {
                r = (await chrome.tabGroups.get(t.groupId)).title;
              } catch {}
            n.push({
              tabId: t.id,
              url: t.url ?? ``,
    ...
    async execute(e) {
      ...
      while (true) {
        ...
        if (D(selector)) {
          ...
          if (refObj) {
            try {
              let { object } = await a(`DOM.resolveNode`, {
                backendNodeId: refObj.backendDOMNodeId,
              });
              if (object && object.objectId) {
                if (state === `attached`) {
                  isMatch = true;
                } else if (state === `visible`) {
                  let box = await a(`DOM.getBoxModel`, { objectId: object.objectId }).catch(
                    () => null,
                  );
        ...
        if (Date.now() - startTime > timeout) {
          throw Error(
            `wait_for: timeout after ${timeout}ms waiting for ${selector} to be ${state}`,
          );
        }
  ...
  class HoverTool {
    ...
    async objectIdFromSelector(e) {
      ...
      if (t.result.subtype === `null` || !t.result.objectId)
        throw Error(`hover: element not found: ${e}`);
      return t.result.objectId;
    }
  }
      ...
      try {
        while (true) {
          ...
          if (now - startTime > timeout) {
            throw Error(
              `wait_for_network_idle: timeout after ${timeout}ms with ${inflight.size} requests in flight`,
            );
          }
  ...
  class DragDropTool {
    ...
    async execute(e) {
      ...
      await i((await s()).id);
      let srcObj = D(source)
        ? await this.objectIdFromRef(source)
        : await this.objectIdFromSelector(source);
      let destObj = D(target)
        ? await this.objectIdFromRef(target)
        : await this.objectIdFromSelector(target);
      ...
      return {
        success: true,
        from: { x: Math.round(srcX), y: Math.round(srcY) },
        to: { x: Math.round(destX), y: Math.round(destY) },
      };
    }
    async objectIdFromRef(e) {
      let t = E(e);
    ...
    async objectIdFromSelector(e) {
      ...
      if (t.result.subtype === `null` || !t.result.objectId)
        throw Error(`drag_drop: element not found: ${e}`);
      return t.result.objectId;
    }
  }
  ...
  class DoubleClickTool {
    ...
    async objectIdFromSelector(e) {
      ...
      if (t.result.subtype === `null` || !t.result.objectId)
        throw Error(`double_click: element not found: ${e}`);
      return t.result.objectId;
    }
  }
  ...
  class RightClickTool {
    ...
    async objectIdFromSelector(e) {
      ...
      if (t.result.subtype === `null` || !t.result.objectId)
        throw Error(`right_click: element not found: ${e}`);
      return t.result.objectId;
    }
  }
  ...
  class FocusTool {
    ...
    async objectIdFromSelector(e) {
      ...
      if (t.result.subtype === `null` || !t.result.objectId)
        throw Error(`focus: element not found: ${e}`);
      return t.result.objectId;
    }
  }
  ...
  class SetCookieTool {
    ...
    async execute(e) {
      ...
      let value = e.value;
      let domain = e.domain;
      if (!name || value === undefined || !domain)
        throw Error(`set_cookie: name, value, and domain are required`);
      let path = e.path || `/`;
    ...
    async execute(e) {
      ...
      if (cmd === `get`) {
        ...
        if (!key || value === undefined)
          throw Error(`local_storage: key and value are required for set`);
  ...
  class SetGeolocationTool {
    ...
    async execute(e) {
      ...
      if (lat === undefined || lon === undefined)
        throw Error(`set_geolocation: latitude and longitude are required`);
  ...
  class GetAttributeTool {
    ...
    async execute(e) {
      ...
      if (!selector || !attribute)
        throw Error(`get_attribute: selector and attribute are required`);
  ...
  class SetAttributeTool {
    ...
    async execute(e) {
      ...
      if (!selector || !attribute || value === undefined)
        throw Error(`set_attribute: selector, attribute, and value are required`);
  ...
  class BlockUrlsTool {
    ...
    async execute(e) {
      ...
      if (!patterns || !Array.isArray(patterns))
        throw Error(`block_urls: patterns must be an array of strings`);
  ...
  class LocalAiTool {
    ...
    async execute(e) {
      ...
      if (typeof ai === `undefined` || !ai.languageModel) {
        throw Error(
          `local_ai: Chrome Built-in AI (Gemini Nano) is not enabled or supported on this browser version. Make sure chrome://flags/#prompt-api-for-gemini-nano is enabled.`,
        );
      }
  ...
  class LocalSummarizeTool {
    ...
    async execute(e) {
      ...
      if (typeof ai === `undefined` || !ai.summarizer) {
        throw Error(
          `local_summarize: Chrome Built-in AI Summarizer API is not enabled or supported. Enable chrome://flags/#optimization-guide-on-device-model.`,
        );
      }
  ...
  class YoutubeTranscriptTool {
    ...
    async execute(e) {
      ...
      return {
        transcript: res.result?.value || `Transcript not found or video has no transcript enabled`,
      };
    }
  }
  ...
  class CrossTabExtractTool {
    ...
    async execute(e) {
      ...
      if (!tabIds || !Array.isArray(tabIds))
        throw Error(`cross_tab_extract: tab_ids array is required`);
  ...
  var ve = `ws://127.0.0.1:10087/ws`,
    ...
    J = new (class {
      ...
      async connect(e) {
        ...
        ((this.currentUrl = t),
          await chrome.storage.session.set({ [q.SHOULD_RECONNECT]: !0, [q.WS_URL]: t }),
          t && (await chrome.storage.local.set({ local_url: t })));
        try {
          ...
          nativePort.onMessage.addListener((msg) =>
            e.emit('message', { data: JSON.stringify(msg) }),
          );
          ...
          (e.addEventListener(`open`, () => {
            ...
            e.addEventListener(`close`, () => {
              this.socket === e &&
                ...
                !this.isDisconnecting && this.shouldReconnect && this.scheduleReconnect());
            }),
      ...
      async testConnection(e) {
        return new Promise((t) => {
          ...
          try {
            ...
            t({ ok: !1, reason: e?.message || `invalid url` });
            return;
          }
          let r = setTimeout(() => {
              try {
      ...
      async disconnect() {
        ((this.isDisconnecting = !0),
          ...
          this.clearReconnectTimer(),
          await chrome.storage.session.set({ [q.SHOULD_RECONNECT]: !1 }),
          await chrome.alarms.clear(K),
      ...
      async reconnectIfNeeded() {
        ...
        if (e[q.SHOULD_RECONNECT] && e[q.WS_URL]) {
          ((this.shouldReconnect = !0), await this.connect(e[q.WS_URL]));
          return;
        }
        let t = await chrome.storage.local.get([`local_url`]);
        if (t.local_url) {
          ((this.shouldReconnect = !0), await this.connect(t.local_url));
          return;
        }
      ...
      async scheduleReconnect() {
        ...
        ((this.reconnectTimer = setTimeout(() => {
          ...
          await chrome.alarms.create(K, { delayInMinutes: 1 }));
      }
      clearReconnectTimer() {
      ...
      async handleMessage(e) {
        ...
        switch (e.type) {
          ...
          case `tool_call`:
            ...
            break;
          default:
            console.log(`[ws] unhandled message type:`, e.type);
        }
      }
      ...
      sendRaw(e) {
        this.socket?.readyState === 1 && this.socket.send(JSON.stringify(e));
      }
    })(),
          ...
          (e, t, n) => (
            (async () => {
              try {
                switch (e.type) {
                  case `GET_STATUS`:
                    n({ connected: J.isConnected(), serverUrl: J.getServerUrl() });
                    break;
                  case `CONNECT`:
                    (await J.connect(e.url), n({ success: !0 }));
                    break;
                  case `DISCONNECT`:
                    (await J.disconnect(), n({ success: !0 }));
                    break;
                  case `TEST_CONNECTION`:
                    n(await J.testConnection(e.url));
                    break;
  ...
  var X = class {
      constructor(e) {
        if (e === `<all_urls>`)
          ((this.isAllUrls = !0),
            (this.protocolMatches = [...X.PROTOCOLS]),
            (this.hostnameMatch = `*`),
            (this.pathnameMatch = `*`));
        else {
          let t = /(.*):\/\/(.*?)(\/.*)/.exec(e);
  ...
  function xe(e, t) {
    if (!Z.PROTOCOLS.includes(t) && t !== `*`)
      throw new Q(e, `${t} not a valid protocol (${Z.PROTOCOLS.join(`, `)})`);
  }
  function Se(e, t) {
    if (t.includes(`:`)) throw new Q(e, `Hostname cannot include a port`);
    if (t.includes(`*`) && t.length > 1 && !t.startsWith(`*.`))
      throw new Q(e, `If using a wildcard (*), it must go at the start of the hostname`);
  }
