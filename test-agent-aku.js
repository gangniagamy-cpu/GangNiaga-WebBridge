...
/**
 * Local WebBridge Test Harness / Agent for WSL (hermes-agent-aku)
...
const DAEMON_HOST = (() => {
  ...
  try {
    ...
    // Fall back to localhost if IP detection fails
  }
  return 'localhost';
})();
...
const API_KEY = (() => {
  if (
    ...
    process.env.GANGNIAGA_API_KEY !== 'your_generated_api_key_here' &&
    process.env.GANGNIAGA_API_KEY !== 'your-api-key-here'
  ) {
